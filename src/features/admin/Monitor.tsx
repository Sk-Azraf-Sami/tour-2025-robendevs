...
export default function Monitor() {
  ...
  const [selectedTeam, setSelectedTeam] = useState<TeamMonitoringData | null>(
    null
  );
  ...
  useEffect(() => {
    // Fetch teams and game status
    fetchTeamsData();

    // Fetch puzzles for checkpoint name mapping
    FirestoreService.getAllPuzzles()
      .then(setPuzzles)
      .catch(() => message.error("Failed to fetch puzzles"));
  ...
  const executeGameControl = async (
    action: "start" | "pause" | "resume" | "reset"
  ) => {
  ...
  const columns = [
      ...
      render: (_: unknown, record: TeamMonitoringData) => (
        <div className="w-32">
          <Text strong className="text-sm">
            {`${puzzleNameMap[record.currentCheckpoint] || record.currentCheckpointName}`}
          </Text>
          <br />
    ...
    },
    {
            ...
            <div className="text-xs text-gray-500 space-y-1">
              ...
              <div className="flex justify-center gap-1 text-xs">
                <span style={{ color: "#1890ff" }}>
                  MCQ: {record.currentLegDetails.mcqPoints}
                </span>
                <span style={{ color: "#52c41a" }}>
                  +P: {record.currentLegDetails.puzzlePoints}
                </span>
                <span
                  style={{
                    color:
                      record.currentLegDetails.timeBonus >= 0
                        ? "#52c41a"
                        : "#ff4d4f",
                  }}
                >
                  +T: {record.currentLegDetails.timeBonus >= 0 ? "+" : ""}
                  {record.currentLegDetails.timeBonus}
                </span>
              </div>
            </div>
          )}
      ...
      sorter: (a: TeamMonitoringData, b: TeamMonitoringData) =>
        ...
        <div className="text-center">
          ...
          {record.gameStartTimeFormatted && (
            <Text className="text-xs text-gray-500 block">
              Started:{" "}
              {new Date(record.gameStartTimeFormatted).toLocaleTimeString()}
            </Text>
          )}
        </div>
      ),
    ...
    },
    {
      ...
      render: (status: string, record: TeamMonitoringData) => (
        <div className="text-center">
          ...
          {record.timeSinceLastScan &&
            record.timeSinceLastScan > 600 &&
            record.status === "in_progress" && (
              <Text className="text-xs text-orange-500 block">
                Slow progress
              </Text>
            )}
        </div>
      ...
      onFilter: (value: unknown, record: TeamMonitoringData) =>
        record.status === value,
    },
    {
      title: "Actions",
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 pb-3 sm:pb-4 space-y-3 sm:space-y-0">
        <div>
          ...
          <Text className="text-gray-600 text-sm sm:text-base">
            Real-time monitoring of team progress and game status
          </Text>
        </div>
          ...
          <Col xs={12} sm={6} key={index}>
            <Card className={`${stat.bgColor} border ${stat.borderColor}`}>
              <div className="text-center">
                ...
                <br />
                <Text className={`text-xs sm:text-sm ${stat.color} truncate`}>
                  {stat.title}
                </Text>
              </div>
            </Card>
        ...
        title={
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm sm:text-base">
              Team Details: {selectedTeam?.username}
            </span>
          </div>
        }
        placement="right"
      ...
      >
        {selectedTeam && (
          <div className="space-y-6">
            ...
            <Card size="small" title="Team Overview">
              <Descriptions column={1} size="small">
                ...
                <Descriptions.Item label="Status">
                  <Badge
                    status={
                      getStatusColor(selectedTeam.status) as
                        | "success"
                        | "processing"
                        | "warning"
                        | "error"
                ...
                <Descriptions.Item label="Total Time">
                  <Text code className="text-lg">
                    {selectedTeam.totalTimeFormatted}
                  </Text>
                </Descriptions.Item>
            ...
            {selectedTeam.status === "in_progress" &&
              selectedTeam.currentLegDetails && (
                <Card size="small" title="Current Checkpoint">
                  <Descriptions column={1} size="small">
                    ...
                    <Descriptions.Item label="Time on Checkpoint">
                      <Text code>
                        {selectedTeam.timeOnCurrentCheckpoint || "0:00"}
                      </Text>
                      {selectedTeam.timeSinceLastScan &&
                        selectedTeam.timeSinceLastScan > 600 && (
                    ...
                    <Descriptions.Item label="Current Points">
                      <Space>
                        <Text>
                          MCQ: {selectedTeam.currentLegDetails.mcqPoints}
                        </Text>
                        <Text>
                          Puzzle: {selectedTeam.currentLegDetails.puzzlePoints}
                        </Text>
                        <Text>
                          Time Bonus: {selectedTeam.currentLegDetails.timeBonus}
                        </Text>
                      </Space>
                    </Descriptions.Item>
                    <Descriptions.Item label="Status">
              ...
              <Card size="small" title="Last Completed Checkpoint">
                <Descriptions column={1} size="small">
                  ...
                  <Descriptions.Item label="Time Taken">
                    <Text code>
                      {selectedTeam.lastCompletedCheckpoint.timeTaken}s
                    </Text>
                  </Descriptions.Item>
                  <Descriptions.Item label="Points Earned">
                    <Text strong className="text-green-600">
                      {selectedTeam.lastCompletedCheckpoint.totalPoints}
                    </Text>
                  </Descriptions.Item>
                </Descriptions>
              </Card>
            ...
            {selectedTeamDetails &&
              selectedTeamDetails.legs &&
              selectedTeamDetails.legs.length > 0 && (
                <Card
                  size="small"
                  title={
                    <div className="flex items-center gap-2">
                  ...
                  extra={
                    <Text className="text-xs text-gray-500">
                      {
                        selectedTeamDetails.legs.filter(
                          (leg) => leg.endTime && leg.endTime > 0
                        ).length
                      }{" "}
                      / {selectedTeamDetails.legs.length} completed
                    </Text>
                  }
                >
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="timing-section bg-gray-50 p-2 rounded text-xs">
                            ...
                            <div className="mt-2 text-center">
                              <Text strong className="text-sm">
                                Duration: {formatDuration(leg.timeTaken)}
                              </Text>
                            </div>
                          </div>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <Tag
                              color={
                                leg.isFirstCheckpoint ? "orange" : "default"
                              }
                              className="text-xs"
                            >
                  ...
                  <div className="hidden sm:block overflow-x-auto">
                    <Table
                      ...
                      dataSource={selectedTeamDetails.legs.map(
                        (leg, index) => ({ ...leg, key: index })
                      )}
                      pagination={false}
                      size="small"
                      scroll={{ x: 700 }}
                  ...
                  <div className="mt-4 summary-stats-grid">
                    <div className="summary-stat-item">
                      <div className="summary-stat-value text-blue-600">
                        {selectedTeamDetails.legs.reduce(
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          >
                            ...
                            {puzzleNameMap[checkpoint.checkpoint]}
                          </Text>
                          {checkpoint.status === "current" && (
              ...
              <Row gutter={[16, 16]}>
                <Col span={12}>
                  <Statistic
                    ...
                    value={
                      selectedTeam.roadmapProgress.filter(
                ...
                <Col span={12}>
                  <Statistic
                    ...
                    value={
                      selectedTeam.roadmapProgress.filter((r) => r.points)
                        .length > 0
                        ? Math.round(
                            selectedTeam.roadmapProgress
                              .filter((r) => r.points)
                ...
                <Col span={12}>
                  <Statistic
                    title="Average Time"
                    value={
                      selectedTeam.roadmapProgress.filter((r) => r.timeTaken)
                        .length > 0
                        ? Math.round(
                            selectedTeam.roadmapProgress
